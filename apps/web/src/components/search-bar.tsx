...
export function SearchBar({ onResultClick, className }: SearchBarProps) {
  ...
  return (
    <div className={cn("relative", className)}>
      ...
      <div
        className={cn(
          "flex items-center gap-2 rounded-lg border bg-[#0C1224]/80 px-2 lg:px-3 py-1.5 text-xs text-white/70 shadow-[0_0_20px_rgba(15,23,42,0.8)] backdrop-blur-sm transition-all",
          isOpen || query
            ? "border-[#3B82F6]/50 bg-[#0C1224]"
            : "border-white/10"
        )}
      >
      ...
      {isOpen && (query || results.length > 0 || loading) && (
        <div
          ref={dropdownRef}
          className="absolute top-full left-0 right-0 mt-2 max-h-[400px] overflow-y-auto rounded-lg border border-white/10 bg-[#121A30] shadow-xl z-50"
        >
          {loading && (
              ...
              {results.map((result, index) => (
                ...
                >
                  <div className="flex items-start gap-3">
                    {result.imageUrl && (
                      <div className="mt-0.5 h-8 w-8 shrink-0 overflow-hidden rounded border border-white/10 bg-white/5">
